...
class Hand(object):
    ...
    def update(self, word):
        ...
        for char in self.handDict:
            new_hand[char] = self.handDict[char]-freq.get(char,0)
        self.handDict = new_hand
    ...
    def __eq__(self, other):
        ...
        for key, value in self.handDict.items():
            if other.handDict.get(key, 0) != value:
                return False
    ...
    def __str__(self):
        ...
        """
        my_string = ''
        for letter in list(self.handDict.keys()):
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        for word in wordlist.getList():
            point = getWordScore(word)
            if self.hand.containsLetters(word) and point > max_score:
                max_score = point
                best_word = word
        return best_word
